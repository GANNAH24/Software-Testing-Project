...
// fill from backend later if needed
const specialties = [];
const locations = [];
const timeSlots = [];
  ...
  useEffect(() => {
    ...
    const loadDoctors = async () => {
      ...
      try {
        ...
        // ✅ Normalize IDs here too
        const normalizedDoctors = doctorList.map(d => ({
          ...d,
          id: d.user_id || d.doctor_id || d.id,
        }));
        ...
        if (doctorId) {
          const doctor = doctorList.find(
            (d) => d.doctor_id === doctorId || d.user_id === doctorId
          );
          if (doctor) {
  ...
  const handleSubmit = async () => {
    ...
    try {
      const appointmentData = {
        doctor_id: selectedDoctor.doctor_id || selectedDoctor.user_id,
        date: selectedDate,
              ...
              {i < 2 && (
                <div
                  className={`h-0.5 flex-1 -mt-7 ${
                    step > s.num ? "bg-[#667eea]" : "bg-gray-200"
                  }`}
                />
              )}
            </div>
        ...
        <div>
          ...
          {errors.doctor && (
            <Alert variant="destructive" className="mb-6">
              <AlertDescription>{errors.doctor}</AlertDescription>
            </Alert>
          )}
          ...
          <Card className="p-6 mb-6 border border-gray-200">
            <div className="flex items-center gap-4 mb-6">
              ...
              <div className="flex-1">
                <p className="text-gray-600 text-sm">Selected Doctor</p>
              ...
              {availableDates.map((date) => {
                ...
                return (
                  ...
                  >
                    <p
                      className={`text-sm ${
                        isSelected ? "text-[#667eea]" : "text-gray-600"
                      }`}
                    >
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-3 mb-6">
              {timeSlots.length === 0 && (
                <p className="text-gray-500 text-sm col-span-full">
                  No time slots available. Fetch them from backend.
                </p>
              )}
                  ...
                  >
                    <Clock
                      className={`w-5 h-5 mx-auto mb-2 ${
                        isSelected ? "text-[#667eea]" : "text-gray-400"
                      }`}
                    />
                    <p
                      className={`${
