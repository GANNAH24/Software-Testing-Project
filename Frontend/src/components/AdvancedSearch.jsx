    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        {searchResults !== null && (<div><div className="mb-6"><h2 className="text-gray-900">{searchResults.length} {searchResults.length === 1 ? 'doctor' : 'doctors'} found</h2></div>{searchResults.length === 0 ? (
          ...
          <Card key={doctor.doctor_id || doctor.id} className="hover:shadow-md transition-shadow"><CardContent className="p-6"><div className="flex items-start justify-between mb-4"><div className="flex-1"><h3 className="text-gray-900 mb-1">{doctor.name}</h3><p className="text-[#667eea]">{doctor.specialty}</p></div><div className="flex items-center gap-1 bg-yellow-50 px-2 py-1 rounded"><Star className="w-4 h-4 text-yellow-500 fill-yellow-500" /><span className="text-sm text-gray-900">{doctor.reviews_count || doctor.reviewsCount || 0}</span></div></div><div className="space-y-2 mb-4"><p className="text-sm text-gray-600">{doctor.qualifications}</p><div className="flex items-center gap-2 text-sm text-gray-600"><MapPin className="w-4 h-4" /><span>{doctor.location}</span></div></div><Button onClick={() => navigate('doctor-profile', { doctorId: doctor.doctor_id || doctor.id })} className="w-full bg-[#667eea] hover:bg-[#5568d3] gap-2">View Profile<ChevronRight className="w-4 h-4" /></Button></CardContent></Card>
        ))}</div>)}</div>)}
      </div>
    </div>
  );
