...
export function ManagePatients({ navigate, user }) {
  ...
  const handleUpdate = async () => {
    ...
    try {
      console.log('Updating patient with ID:', selectedPatient.patient_id || selectedPatient.id);
      console.log('Form data:', formData);
      // Assuming 'patient_id' based on the doctor pattern
      const response = await adminService.updatePatient(selectedPatient.patient_id || selectedPatient.id, formData);
  ...
  const confirmDelete = async () => {
    ...
    try {
      await adminService.deletePatient(patientToDelete.patient_id || patientToDelete.id);
                ...
                <tbody>
                  {patients.map(patient => (
                    <tr key={patient.patient_id || patient.id} className="border-b border-gray-200 last:border-0">
                      <td className="p-4 text-gray-900">{patient.full_name}</td>
                      ...
                      <td className="p-4"><div className="flex gap-2">
                        ...
                        <Button size="sm" variant="destructive" onClick={() => handleDelete(patient)}>Delete</Button>
                      </div></td>
                    </tr>
                  ))}
                </tbody>
            ...
            <div className="md:hidden space-y-4">
              {patients.map(patient => (
                <Card key={patient.patient_id || patient.id}><CardContent className="p-4">
                  <div className="mb-3"><div className="text-gray-900">{patient.full_name}</div><div className="text-sm text-gray-600 capitalize">{patient.gender}</div></div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <Button size="sm" variant="destructive" onClick={() => handleDelete(patient)} className="flex-1">Delete</Button>
                  </div>
                </CardContent></Card>
              ))}
            </div>
        ...
        <Dialog open={updateDialogOpen} onOpenChange={setUpdateDialogOpen}>
          <DialogContent>
            ...
            <div className="space-y-4">
              ...
              <div><Label htmlFor="updateEmail">Email (Read-only)</Label><Input id="updateEmail" type="email" value={formData.email} disabled className="mt-2 bg-gray-100" /></div>
