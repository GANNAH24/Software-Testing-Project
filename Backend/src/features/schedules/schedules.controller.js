...
const createSchedule = asyncHandler(async (req, res) => {
    ...
    try {
        ...
        console.log('Schedule creation error:', err.message);
...
const getAvailableSlots = asyncHandler(async (req, res) => {
    ...
    // Get existing appointments for this doctor on this date to exclude booked slots
    ...
    const bookedSlots = new Set((existingAppointments || []).map(apt => apt.time_slot));
    ...
    let availableSlots = schedules
        .filter(s => s.is_available && s.time_slot && !bookedSlots.has(s.time_slot))
        .map(s => s.time_slot);
