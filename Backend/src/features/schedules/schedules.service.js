...
const getWeeklySchedule = async (doctorId, date) => {
    ...
    const schedule = await schedulesRepository.getWeeklySchedule(doctorId, weekStart);
...
const blockTime = async (doctorId, date, timeSlot, reason = null) => {
    ...
    for (const sch of existing) {
        ...
        if (sch.is_available === false && overlaps(existingStart, existingEnd, target.start, target.end)) {
            const conflictRange = sch.time_slot || `${existingStart}-${existingEnd}`;
            const err = new Error(`Requested time ${timeSlot} overlaps an already-blocked slot: ${conflictRange}`);
            err.statusCode = 400;
            throw err;
        }
    }
    ...
    for (const sch of existing) {
        ...
        if (overlaps(existingStart, existingEnd, target.start, target.end) && sch.is_available) {
